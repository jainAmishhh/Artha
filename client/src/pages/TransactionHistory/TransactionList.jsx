...
//             return (
//               <div key={t._id} className="group flex items-center justify-between p-5 bg-white border border-slate-100 rounded-2xl hover:shadow-md transition-all">
//                 <div className="flex items-center gap-4 flex-1">
...
//                       <span>{t.category}</span> • <span className="flex items-center gap-1"><Calendar size={14} /> {formatDate(t.date)}</span>
//                     </div>
//                   </div>
//                 </div>
//                 <div className={`flex items-center gap-2 px-4 py-2 rounded-xl font-bold ${isIncome ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'}`}>
//                   {isIncome ? <ArrowUpRight size={18} /> : <ArrowDownLeft size={18} />}
//                   <span>{isIncome ? '+' : ''}₹{t.amount.toLocaleString()}</span>
//                 </div>
//               </div>
//             );
//           })
