...
const FormContainer = ({ onLogin }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // response already saved to localStorage in formHandlers
  ...
  const onVerifyOtp = async (enteredOtp) => {
    try {
      ...
      // Passed the response 'res' to onLogin so the parent app has the user data
  ...
  // Handler for when Google Login completes
  const onGoogleAuthSuccess = (userObject) => {
      // Data is already saved to localStorage in GoogleLoginForm
      if (onLogin) onLogin(userObject);
      navigate("/");
  };
    ...
    <div className="bg-white rounded-3xl p-8 shadow-2xl border border-slate-200">
      ...
      <div className="space-y-6">
        ...
        {loginMethod === "google" && (
            <GoogleLoginForm 
                isLogin={isLogin}
                onGoogleAuth={onGoogleAuthSuccess}
        ...
        {loginMethod === "number" && (
          <NumberLoginForm
            ...
            formData={formData}
            handleInputChange={handleInputChange}
            handleSubmit={handleSubmit} // This is used if there's a final submit after OTP
